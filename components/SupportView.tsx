...
export const SupportView: React.FC = () => {
  ...
  const [supportState, setSupportState] = useState<'ai' | 'connecting' | 'queue' | 'connected'>('ai');
  ...
  // Handle scroll position detection to determine whether to auto-scroll on new messages
  const handleScroll = () => {
  ...
  // Tawk.to script initialization ready when connected and property ID is configured
  useEffect(() => {
    const propertyId = process.env.NEXT_PUBLIC_TAWK_PROPERTY_ID;
    const widgetId = process.env.NEXT_PUBLIC_TAWK_WIDGET_ID || 'default';
    if (propertyId && (supportState === 'connected' || activeTabMode === 'live')) {
  ...
  // Periodic check for 15 minutes inactivity while tab is open
  useEffect(() => {
  ...
  const sendMessageToApi = async (textToSend: string) => {
    ...
    try {
      ...
      if (data.isHumanHandoff) {
        setSupportState('connecting');
        setTimeout(() => {
          const propertyId = process.env.NEXT_PUBLIC_TAWK_PROPERTY_ID;
          if (propertyId) {
            setSupportState('connected');
          } else {
            setSupportState('queue');
            setMessages(prev => [
              ...prev,
              {
                sender: 'ai',
                text: "Our live support team is currently assisting other institutional clients. Your conversation has been placed in the support queue. Please remain connected or continue with AI Support.",
                timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
              }
            ]);
          }
  ...
  const hasTawkProperty = Boolean(process.env.NEXT_PUBLIC_TAWK_PROPERTY_ID);
  ...
  return (
    <div className="space-y-6 pb-24 sm:pb-12 animate-fadeIn max-w-4xl mx-auto">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[580px]">
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 space-y-3">
              ...
              <p className="text-xs max-w-sm text-slate-400 leading-relaxed">
                Type your question below regarding withdrawals, deposits, account status, trading, or security. You can also attach a screenshot for analysis.
              </p>
            </div>
          ) : (
        ...
        {/* Live Support Handoff Action Prompt if requested */}
        {supportState === 'connected' && hasTawkProperty && (
          <div className="px-4 py-3 bg-emerald-500/10 border-t border-emerald-500/20 text-emerald-400 text-xs flex items-center justify-between">
            <span className="font-medium">Live human agent connection active via Tawk.to</span>
            <span className="font-mono text-[11px]">Secure Channel</span>
          </div>
        )}
        ...
        <form onSubmit={handleSendMessage} className="p-3 sm:p-4 bg-slate-950/90 border-t border-slate-800 flex items-center gap-2 sm:gap-3">
          ...
          <input 
            ...
            placeholder="Type your question or request a human agent..."
